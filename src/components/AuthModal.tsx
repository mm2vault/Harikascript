...
export const AuthModal: React.FC<AuthModalProps> = ({isOpen,onClose}) => {
  ...
  const submit=async()=>{
    if(!email.trim()||!password){setMessage('E-posta ve şifre gerekli.');return;}
    ...
    try{
      if(mode==='signup'){
        ...
        onClose();
      }else{await signInWithEmailAndPassword(firebaseAuth,email.trim(),password);onClose();}
  ...
  const google=async()=>{
    ...
    try{await signInWithPopup(firebaseAuth,new GoogleAuthProvider());onClose();}
    catch(e:any){setMessage(e?.message||'Google ile giriş başlatılamadı.');setBusy(false);}
  };
  return <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
    <div className="w-full max-w-md rounded-3xl bg-[#0d1019] border border-white/10 p-6 shadow-2xl">
      <div className="flex justify-between items-center"><div><h2 className="text-xl font-bold text-white">{mode==='login'?'Giriş Yap':'Hesap Oluştur'}</h2><p className="text-xs text-slate-400 mt-1">Google veya e-posta + şifre.</p></div><button onClick={onClose} className="w-9 h-9 rounded-full bg-white/5 text-slate-400 flex items-center justify-center"><X className="w-4 h-4"/></button></div>
      <button onClick={google} disabled={busy} className="mt-5 w-full rounded-xl bg-white text-slate-900 py-3 text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50"><Chrome className="w-4 h-4"/> Google ile devam et</button>
