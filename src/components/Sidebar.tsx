...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navSections: {
    heading: string;
    items: { id: CategoryType; label: string; icon: React.ReactNode; badge?: string }[];
  }[] = [
    {
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 text-left relative ${
                      isActive
                        ? 'bg-[#15142a] text-white border border-indigo-500/40 shadow-[0_0_20px_rgba(99,102,241,0.15)]'
                        : 'text-slate-400 hover:text-slate-200 hover:bg-white/[0.03]'
                    }`}
                  >
                    {/* Active glow indicator on left */}
                    {isActive && (
                      <span className="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-gradient-to-b from-indigo-400 to-purple-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]" />
                    )}
      ...
      <div className="mt-8">
        <div className="relative rounded-2xl p-4 overflow-hidden border border-indigo-500/30 bg-gradient-to-b from-[#16142a] via-[#100e20] to-[#0d0c18] shadow-lg">
          ...
          <p className="text-xs text-slate-400 mt-1 leading-relaxed">
            {isPremium
              ? 'Tüm VIP avantajlarından ve ekstra indirimlerden yararlanıyorsun.'
              : 'VIP scriptler ve özel Discord kozmetikleri seni bekliyor!'}
          </p>

          <button
            id="btn-sidebar-premium"
            onClick={onOpenPremium}
            className="mt-3.5 w-full py-2 px-3 rounded-xl bg-[#1d1b38] hover:bg-[#252247] border border-indigo-500/30 hover:border-indigo-400/50 text-xs font-semibold text-indigo-200 hover:text-white flex items-center justify-between transition-all group"
          >
